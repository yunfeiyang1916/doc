...
func initUserOpSrvConn() {
	host := global.ServerConfig.UserOpSrvInfo.Host
	port := global.ServerConfig.UserOpSrvInfo.Port

	// 初始化连接池
	pool, err := grpc_pool.New(fmt.Sprintf("%s:%d", host, port), grpc_pool.DefaultOptions)
	if err != nil {
		zap.S().Fatal("[initUserOpSrvConn] 初始化库存服务连接池失败")
	}
	global.UserOpSrvConnPool = pool
}
