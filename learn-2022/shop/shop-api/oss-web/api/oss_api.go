...
func get_policy_token() string {
	...
	// 用户上传文件时指定的前缀。
	var upload_dir = global.ServerConfig.OssInfo.UploadDir
	...
	var accessKeySecret = global.ServerConfig.OssInfo.ApiSecret
	...
	var callbackUrl = global.ServerConfig.OssInfo.CallBackUrl
	...
	// 请填写您的AccessKeyId。
	var accessKeyId = global.ServerConfig.OssInfo.ApiKey
	policyToken.AccessKeyId = accessKeyId
	// host的格式为 bucketname.endpoint ，请替换为您的真实信息。
	var host = global.ServerConfig.OssInfo.Host //"http://bucket-name.oss-cn-hangzhou.aliyuncs.com"
...
func Callback(c *gin.Context) {
	...
	// Get PublicKey bytes
	bytePublicKey, err := getPublicKey(c.Request)
	if err != nil {
		responseFailed(c.Writer)
		return
	}

	// Get Authorization bytes : decode from Base64String
	byteAuthorization, err := getAuthorization(c.Request)
	if err != nil {
		responseFailed(c.Writer)
		return
	}
	...
	byteMD5, err := getMD5FromNewAuthString(c.Request)
	if err != nil {
		responseFailed(c.Writer)
		return
	}
	...
	if verifySignature(bytePublicKey, byteMD5, byteAuthorization) {
		...
		responseSuccess(c.Writer) // response OK : 200
	} else {
		responseFailed(c.Writer) // response FAILED : 400
	}
}
