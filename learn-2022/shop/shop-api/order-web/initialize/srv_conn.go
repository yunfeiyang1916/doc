...
func InitSrvConn() {
	initOrderSrvConn()
	initGoodsSrvConn()
	initInventorySrvConn()
}

func initOrderSrvConn() {
	host := global.ServerConfig.OrderSrvInfo.Host
	port := global.ServerConfig.OrderSrvInfo.Port
	// 初始化连接池
	pool, err := grpc_pool.New(fmt.Sprintf("%s:%d", host, port), grpc_pool.DefaultOptions)
	if err != nil {
		zap.S().Fatal("[InitSrvConn] 初始化订单服务连接池失败")
	}
	global.OrderSrvConnPool = pool
}
...
func initInventorySrvConn() {
	host := global.ServerConfig.InventorySrvInfo.Host
	port := global.ServerConfig.InventorySrvInfo.Port

	// 初始化连接池
	pool, err := grpc_pool.New(fmt.Sprintf("%s:%d", host, port), grpc_pool.DefaultOptions)
	if err != nil {
		zap.S().Fatal("[initInventorySrvConn] 初始化库存服务连接池失败")
	}
	global.InventorySrvConnPool = pool
}
